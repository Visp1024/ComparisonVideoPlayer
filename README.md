...
- Правая кнопка на клипе — «Вырезать отрезок в файл…»: отрезок того трека, по
  которому щёлкнули, уходит в отдельное видео рядом с исходником. Точный режим
  перекодирует (границы ровно те, что на ручках), быстрый копирует поток без
  перекодирования (начало отъезжает к ближайшему ключевому кадру). Режут всегда
  исходный файл, даже когда трек играет из кэша кадров.
