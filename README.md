...
- Открыт один ролик — плеер это и показывает: пропадают буквы треков над кадром,
  в счётчике кадров, в шапке и вкладки A / B боковой панели. Таблетка раскладки
  остаётся: сегмент недостающего трека просто неактивен, а «оба кадра» показывает
  открытый ролик рядом с приглашением перетащить второй.
