...
public sealed class FlyleafBackend : IPlaybackBackend
{
    ...
    private static Config CreateConfig()
    {
        ...
        // Записи экрана QuickTime приносят монтажный лист (edit list) со сдвигом начала.
        // Разбирая его «точно» (умолчание ffmpeg), демуксер оставляет start_time дорожки
        // нулевым, а первый пакет отдаёт с отрицательным pts. FlyleafLib сверяет pts
        // первого пакета с началом дорожки, не сходится — и требует ключевой кадр,
        // которого после отбраковки уже не получает: показ кадра по номеру уходит в
        // бесконечный цикл «перемотка назад — декод», и окно намертво зависает.
        // Упрощённый разбор оставляет pts как есть — сдвиг попадает и в start_time,
        // и в пакеты, они снова сходятся. Кадры при этом не теряются.
        cfg.Demuxer.FormatOpt["advanced_editlist"] = "0";
