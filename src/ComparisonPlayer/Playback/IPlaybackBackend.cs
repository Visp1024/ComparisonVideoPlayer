...
public interface IPlaybackBackend : IDisposable
{
    ...
    /// <summary>
    /// Переход на позицию <b>без остановки воспроизведения</b>: этим фаза 3 подтягивает
    /// ведомый трек, разошедшийся с мастером. Обычный seek здесь не годится — он ставит
    /// плеер на паузу, и коррекция дрейфа выглядела бы как рывок с остановкой.
    /// </summary>
    void NudgeTo(TimeSpan position);
}
